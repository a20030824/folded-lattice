...
Each ZIP has the files Lively needs at its archive root:
...
CI runs `npm run build:lively`, validates every ZIP with `unzip`, and uploads all four archives as the `lively-packages` workflow artifact. This provides installable test packages without committing generated output.
