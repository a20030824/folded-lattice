...
- `src/app/main.ts` 只處理瀏覽器入口接線：DOM 查找、URL 訂閱、resize debounce、visibility、debug handles 與 unload cleanup。
- `src/app/presetRuntimeManager.ts` 管理 preset runtime：解析 preset/mode、staging、renderer/engine 建立、pointer/Lively/WebGL recovery 綁定、原子 commit/rollback 與 dispose。
...
`PresetRuntimeManager` 一次只擁有一個 committed runtime。切換時先在 detached canvas 上建立完整 candidate，成功替換 DOM 後才發布新 runtime 並清理舊 runtime；staging 或 commit 失敗時只清理 candidate，舊畫面、engine、Lively listener 與 RAF 保持運作。URL mode 切換與 WebGL context-loss recovery 都走同一條原子切換路徑。詳細順序見 `docs/runtime-lifecycle.md`。
...
`TopologyBuilder` 接收 viewport 與共享 config，回傳 `TopologyBuildResult`。結果包含通用的 `TopologyState`，也可提供 `initializeResources` 來初始化 feature runtime。Delaunay 與 crease topology 都遵守同一個 contract；引擎不需要知道使用哪一種幾何。
...
需要 WebGL fallback 的 preset 在自己的 renderer factory 內替換 canvas 並回傳新的 canvas 與 renderer。app runtime manager 只接收 `PresetRendererResult`，不保存任何作品專屬的渲染判斷。
...
`src/wallpaper/lively.ts` 只負責：讀取 Lively 事件、尋找 binding、執行 binding、管理 debounce，以及將 preset selection 交回 app runtime manager。它不能直接操作 `creatureConfig`、`creaseConfig` 或其他 feature config。
...
`createEngine()` 只負責通用的 animation loop、fixed timestep、resize/rebuild、render 與 dispose。preset 的資料與行為由 `PresetDefinition` 注入；切換 preset 時，runtime manager 先在 detached canvas 建立 candidate，DOM commit 成功後才清理舊 runtime，失敗則保留舊 runtime。